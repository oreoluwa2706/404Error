...
function checkNumber() {
  ...
  if (
    mtnList.some(function (mtnPrefix) {
      return phoneNumber.startsWith(mtnPrefix);
    })
  ) {
    ...
    gMobileList.some(function (gMobilePrefix) {
      return phoneNumber.startsWith(gMobilePrefix);
    })
  ) {
    network.src = "./image/9mobile.png";
    myNetworkName.innerText = "9MOBILE";
  } else if (
    gloList.some(function (gloPrefix) {
      return phoneNumber.startsWith(gloPrefix);
    })
  ) {
    ...
    airtelList.some(function (airtelPrefix) {
      return phoneNumber.startsWith(airtelPrefix);
    })
  ) {
